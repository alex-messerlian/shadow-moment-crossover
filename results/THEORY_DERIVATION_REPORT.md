...
- **Weight-only ansatz `ζ₁ = Σ_P c_{|P|}⟨P⟩²` FAILS at n=2** — confirmed: across a diverse 19-state family (12 noisy-pure at q ∈ {0, 0.1, 0.2, 0.3} × 3 seeds, 4 low-rank at r ∈ {2,3} × 2 seeds, 3 noisy GHZ — see `experiments/theory_single_copy_verify.py`) a single universal (c₁,c₂) fits only to **12.3% max / 5.7% RMS** residual (`theory_derivation.json`: `max_resid` 0.123, `rms_resid` 0.057), which is the 12.3% the paper quotes. (It *appears* to hold within the narrow q=0.1 Haar family — **0.140% max / 0.056% RMS**, against a ζ₁ Monte-Carlo noise floor of 0.062%; see `experiments/weight_only_narrow_family.py` and `results/weight_only_narrow_family.json` — only because those states span a low-dimensional invariant subspace, a trap.) It also fails at n=3 (2.6%).
- **Why no simple closed form:** ζ₁ = Var[Tr(Gρ)] with `E[Tr(GP)Tr(GP')] = 3^{|P|+|P'|}·E_{U,b~ρ}[⟨b|UPU†|b⟩⟨b|UP'U†|b⟩]`; the U-average factorizes per qubit but the outcome sum weights by the diagonal `⟨b|UρU†|b⟩`, which does not factorize in general. **The reason is not entanglement.** Because the snapshots are drawn from ρ, the second moment is cubic in ρ while a weight-only sum of ⟨P⟩² is quadratic in the Pauli expectations; the missing dependence is on the relative orientation of the marginal Bloch vectors through the correlation matrix, an invariant already nonzero for **separable, classically correlated** states (Section 3.4 of the paper). So the coefficients are not universal and depend on the full state. **ζ₁ (n≥2) has no simple closed form and must be computed numerically**; the single-qubit form is the only exact one.
