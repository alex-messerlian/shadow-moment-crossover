...
- **Weight-only ansatz `ζ₁ = Σ_P c_{|P|}⟨P⟩²` FAILS at n=2** — confirmed: across a diverse 19-state family (12 noisy-pure at q ∈ {0, 0.1, 0.2, 0.3} × 3 seeds, 4 low-rank at r ∈ {2,3} × 2 seeds, 3 noisy GHZ — see `experiments/theory_single_copy_verify.py`) a single universal (c₁,c₂) fits only to **12.3% max / 5.7% RMS** residual (`theory_derivation.json`: `max_resid` 0.123, `rms_resid` 0.057). The paper quotes 12.2%, the exact value; 12.3% is this saved Monte-Carlo estimate of it. (It *appears* to hold at 0.0% within the narrow q=0.1 Haar family only because those states span a low-dimensional invariant subspace — a trap.) It also fails at n=3 (2.6%).
