...
The relationship to our result turns on the task, and this is the crux. They study purity *testing*: deciding between two hypotheses whose distinguishing gap shrinks exponentially under noise. We study purity *estimation*: additive error $\varepsilon$ on $\mathrm{Tr}(\rho^k)$ at a fixed precision. The SWAP-test outcome is a bounded random variable, so estimation to a fixed $\varepsilon$ costs $O(1/\varepsilon^2)$ shots regardless of $n$. Their exponential cost comes from the testing task requiring an exponentially small $\varepsilon$ — the hypothesis gap itself — not from the SWAP test being intrinsically exponential for estimation. We do not evade their lower bound; we solve a different problem.

The two results nonetheless share a mechanism, and this is a strength rather than a coincidence. Our Section 4.2 identity — that under a per-qubit channel the $k$-copy test returns exactly $\mathrm{Tr}(\sigma^k)$ with $\sigma = \mathcal{E}^{\otimes n}(\rho)$ — is the finite-$n$ mechanism underneath their asymptotic theorem for SWAP-based protocols. The collective test faithfully reports the purity of the damaged state $\sigma$, and because per-qubit depolarizing drives $\mathrm{Tr}(\sigma^2)$ toward $2^{-n}$ under *both* the pure and the maximally-mixed hypothesis, the testing gap closes. Feeding our exact law into their setup at $\lambda = 0.1$, the gap $\Delta(n,\lambda) = \mathrm{Tr}(\sigma_{\text{pure}}^2) - 2^{-n}$ falls from $0.54$ at $n=2$ to $0.21$ at $n=10$, and the implied SWAP-test shot count $1/\Delta^2$ rises from $3.5$ to $22$, sitting at or above their lower bound at every size we checked — as it must for any admissible strategy. Its asymptotic base $\big(4/(1+3(1-\lambda)^2)\big)^2$ equals their base $4/(1+3(1-\lambda)^4)$ at $\lambda = 0$ and exceeds it at finite noise, the difference controlled exactly by $3\big(1-(1-\lambda)^2\big)^2 \ge 0$; over $n \le 10$ the large prefactor dominates this small-base exponential, so the growth rate measured on that window sits just below $b(\lambda)$ even though $1/\Delta^2$ itself stays above $b(\lambda)^n$ at every size.
