...
The budget threshold \(M^*\) is a finite-budget effect over the sizes and budgets near-term
experiments occupy: past it the error returns to the \(M^{-1/2}\) law, and we claim no improvement
to that asymptotic scaling.
\end{abstract}
...
its base, defined precisely in item (e) below --- is never below \(7\), and a counting argument
...
\qquad \frac{28}{5} = 5.6 ,
\]
with base and prefactor both exact. The lettered blocks below state the general structure this
is one instance of, and the other branches it does not cover.
...
Three weaker statewise checks run over a wider grid, six families and \(n = 2\) to \(6\) at
three budgets with \(24\) trials each. There the exact law predicts per-state RMSE to a median
...
every qubit of every copy. They continue to hold at three times the rates the crossover sweeps of
