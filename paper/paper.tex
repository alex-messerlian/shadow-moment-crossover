...
\cite{huang2020shadows} --- with two known decay regimes
\cite{elben2020mixed, aasen2026limitations} whose state-dependent
projection variances those analyses bound rather than evaluate, leaving
their ratio unconstrained. We evaluate them in closed form for depolarized
Haar-random pure states, locating the budget \(M^* = \zeta_2/(2\zeta_1)\)
at which the effective error exponent crosses over. It grows exponentially
with asymptotic base \(28/5\), fixed by the ensemble-averaged projection
variances and approached from below by our finite-size fits
(\(n = 2\) to \(9\)).

That turns the regime structure into a plug-in criterion, with nothing
fitted to the crossover data, for when a collective two-copy measurement
attains the lower RMSE at equal copy budget. For the range-projected
estimator an experiment would use, it places all \(72\) resolving cells
within one qubit and predicts \(96.7\%\) of the \(123\) swept, over four
ensembles, three held out from the derivation; Section 5.2 gives the
unbiased estimator's figures. The practical consequence survives
projection: at ten qubits the projected estimate is less accurate than a
constant fixed at the midpoint of the interval purity is known a priori to
occupy. The cells are simulation, and for \(k > 2\) the collective side is
a bounded \(\pm 1\) outcome carrying none of the gates its circuit would
require.

We then test the prediction on what the platform reports as a 108-qubit
...
\cite{cai2025biased} rescales the estimator to trade bias against variance
and argues the trade is always worth making in the finite-sample regime.
The projection of
Section~\ref{physical-constraints-and-the-choice-of-estimator} is the
parameter-free member of that family, biased only by the physical range and
requiring nothing to be tuned.
...
in \(d\) and silent about the finite-budget coefficient.
...
PRX Quantum (2026), doi:10.1103/4rkx-xkwy, arXiv:2505.09206.
...
Commun. Phys. (2026), doi:10.1038/s42005-026-02710-8, arXiv:2412.01696.
