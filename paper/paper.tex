...
separate what is a compilation fact from what is a device measurement: they are what a transpiler
returns for a stated circuit against a published coupling map, and they need no device access to
reproduce.

Transpiling the \(k = 2\) destructive test of Section~2.3 onto a published square-lattice map
with a native \(\{\mathrm{CZ}, R_x, R_z\}\) basis, the \(n = 2\) Bell instance uses four CZ
gates --- two to
