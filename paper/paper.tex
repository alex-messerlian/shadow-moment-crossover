...
\title{A finite-size crossover criterion for shadow-based moment\\ estimation, with a hardware case study}
...
both routes leaves those sizes a conservative upper bound. Re-derived for
the range-projected estimator an experiment would actually use, the
criterion is more accurate still: within one qubit in all \(72\) cells that
then resolve, and \(96.7\%\) of the \(123\). Projection also bounds the
single-copy error, and the practical consequence survives it --- at ten
qubits the projected single-copy estimate is less accurate than a constant
fixed at the midpoint of the interval the purity is known a priori to
occupy, a comparison the collective route passes. The cells are
...
108-qubit superconducting processor, with predictions committed to version
control before the corresponding hardware jobs were submitted. Both failed: a
...
purity of a quantum state is beaten by a constant. Purity is known a priori
to lie in \([2^{-n}, 1]\), and a fixed guess at the midpoint of that
interval, which never looks at the data, has RMSE \(0.310\). The
single-copy estimate, projected into the same interval so that the
comparison is as favourable to it as we can make it, has \(0.582\), against
a true purity of \(0.810\). Nor is this a ten-qubit effect: the single-copy
route already loses to that constant at seven qubits, while the collective
two-copy route holds out to ten. Before projection the estimator is still
unbiased; it is the spread that has become useless.
...
We compare two routes. The single-copy route measures one copy at a time in
...
budget. The same exact Hoeffding architecture runs at \(k = 3\) and \(4\)
with independently estimated projection variances --- no closed forms are
claimed at those orders --- where prior treatments bound the higher-moment
variance, Elben et al.'s Appendix D.2 among them, and the two-term
...
\subsection{\texorpdfstring{The \(k = 2\) projection variances in closed
form}{The k = 2 projection variances in closed form}}
...
RMSE of Section~5.2 measures; it is not the ensemble mean of the statewise
...
shot budget, a prediction Section 5.4 tests directly and which holds.
...
\caption{Crossover validation across 83 cells. Predicted versus Monte-Carlo-observed crossover size $n^*$ over moment orders $k\in\{2,3,4\}$, three noise models, and four state ensembles (three unseen by the theory). Accuracy for the unbiased estimator (83 resolving cells) and for the range-projected estimator (72) is reported in Secs.~5.2 and~5.3. The cells are simulation, not hardware; for the higher moment orders the collective side is an idealized bounded-outcome primitive carrying no gate, depth, or routing cost.}
\label{fig:boundary}
\end{figure}
...
We began with a comparison: at ten qubits, on noisy-pure states at a fixed
budget, single-copy estimation of purity --- projected into the range the
purity is known a priori to occupy, which can only help it --- is less
accurate than a constant fixed at the midpoint of that range, and has been
since seven qubits. That is not an accident of implementation, and this
paper explains where it comes from.
...
law predicts, with no parameter fitted to the crossover outcomes, the
size at which collective measurement attains the lower RMSE at equal copy
budget. It places 82 of
the 83 simulated crossovers within one qubit for the unbiased estimator,
and all 72 that still resolve one for the range-projected estimator.
...
lower-RMSE route beyond roughly six to seven qubits --- six for the unbiased
estimator, seven once estimates are projected into the physical range, and
both of the independent noisy-pure sweeps agree on each; the higher moments
...
states per size. The trial count differs between the two noisy-pure sweeps:
the \(60\) cells of \texttt{budget\_scaling.json} use \(8\) Monte Carlo
trials per state (\texttt{run\_budget\_sweep.py}: \texttt{N\_STATES}~\(=48\),
\texttt{N\_TRIALS}~\(=8\)), while the \(36\) cells of
\texttt{moment\_\allowbreak sweep\_\allowbreak corrected.json} use \(10\). The held-out stress ensembles use \(4\) states
...
\((1-q) + q/d\) once and \(q/d\) with multiplicity \(d - 1\), independent of
\(\lvert\psi\rangle\), so
\(\mathrm{Tr}(\rho^k) = \bigl((1-q) + q/d\bigr)^k + (d-1)(q/d)^k\) is a
function of \((n, q, k)\) alone. The same holds trivially for Haar-pure
states, whose moments are all \(1\), and for the single fixed GHZ-noisy
state. Measured over \(48\) states at \(n = 4\), the per-state standard
deviation of \(\mathrm{Tr}(\rho^k)\) is at machine precision on all three
--- at most \(7.1 \times 10^{-16}\) on noisy-pure,
\(1.2 \times 10^{-15}\) on Haar-pure, and \(1.1 \times 10^{-16}\) on
GHZ-noisy, over \(k = 2, 3, 4\). Only the rank-2 Ginibre family has a
genuinely varying estimand, with relative spread \(5.0\%\), \(12.9\%\), and
\(21.9\%\) at \(k = 2, 3, 4\).

Where this paper says a quantity is averaged over \(48\) states, the average
is therefore over estimator \emph{difficulty} at a fixed estimand, not over
distinct estimation problems; that is the weaker of the two readings and it
is the one we mean. The difficulty does vary: the per-state projection
variance \(\zeta_2\) has relative spread \(1.6\%\) at \(n = 4\) on
noisy-pure, because it is a weighted sum over the state's Pauli spectrum
(Section~\ref{sec:zeta-closed-form}) rather than a function of the spectrum
of \(\rho\). The estimator is never given \(q\) or the ensemble, so the
degeneracy is not available to it. What the degeneracy does mean is that
these three ensembles do not test robustness to a varying target, and the
rank-2 family is the only one that does.
...
from the aggregate rather than counted as a hit or a miss. Three distinct
rules produce the numbers reported here, and they are not interchangeable.
\emph{Rule 1} (\texttt{theory/crossover.py}, \texttt{predict\_crossover})
compares the two theory curves and returns the smallest size from which the
single-copy RMSE exceeds the collective RMSE and stays above it for the
remainder of the swept range; it produces every \emph{predicted} \(n^*\).
\emph{Rule 2} (\texttt{benchmark/hardened.py}, \texttt{crossover\_table})
compares the two \emph{measured} curves with a state-level paired
\(z\)-test and returns the first size at which the collective route wins at
\(|z| > 2\), without requiring the win to be sustained; it produces the
measured \(n^*\) of the \(68\) noisy-pure cells. \emph{Rule 3}
(\texttt{run\_stress\_test.py}, part 4) compares the measured single-copy
RMSE against the theory collective floor and requires the crossing to be
sustained; it produces the measured \(n^*\) of the held-out cells, \(15\) of
which enter the \(83\).

The rules agree on \(88\) of the \(96\) noisy-pure cells swept. In seven of
the eight disagreements the paired test is the more conservative of the
two: it declines a call that the sustained-RMSE rule takes, on a margin
statistically indistinguishable from zero (\(|z|\) between \(0.02\) and
\(1.47\); single-copy RMSE at most \(1.06\) times collective), and takes the
crossover one qubit later where the margin is decisive (\(|z|\) between
\(3.7\) and \(12.0\)). The eighth cell is already flagged
\texttt{ambiguous} by the pipeline: the collective route wins at \(n = 2\)
but the two are tied at \(n = 5\), so no sustained crossing exists.
