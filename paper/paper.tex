...
does not. Every \(k \ge 3\) result in this paper is a simulation of an idealized bounded-outcome
primitive that carries none of that cost, and the crossover sizes Section 7 reports for \(k \ge 3\) should be read as an upper bound on
...
Fitting the sampled projection variances over \(n = 2\) to \(7\) gives clean exponential
scalings, retained here for the contrast with the closed forms of
Section~\ref{sec:zeta-closed-form}, which return \(5.18\) for the \(M^*\) base over that
window against the regression's \(5.15\). Over that window,
...
forward-simulation trials per state --- so \(\eta \approx 4.1\%\) against a predicted spread of \(21.9\%\), a
spread-to-noise ratio of \(5.3\) rather than the \(4.58\) tabulated above, because this run
...
statewise \(M^*\) for the same state, on the noisy-pure family. The error falls at least as fast as \(M^{-1/2}\), the rate an unbiased variance estimate
should show, and the budget needed for a given accuracy
