...
model used for all \(k\) in Section 7. The cost is not comparable: Section~\ref{sec:compilation} gives the transpiled
...
primitive that carries none of that cost, and the reader should read those sizes as an upper bound
on what the collective route buys.
...
counted in Appendix~A. Only the identity term is state-independent, so the base of \(\zeta_2\)
...
\qquad \frac{28}{5} = 5.6 ,
\]
with base and prefactor both exact. The three paragraphs below state the
general structure this is one instance of, and the other branches it does not
cover.

The fitted base drifts with the fitting window, from \(5.14\) over
\(n = 2\) to \(7\), through \(5.34\) over \(n = 2\) to \(9\), to \(5.60\)
...
floor while the single-copy RMSE keeps falling, a direct test of the
bias-versus-variance distinction the whole crossover rests on.
...
two states of identical purity can be deformed by different amounts. Purity
(\(k=2\)) crosses at \(n \approx 6\) to \(7\) under realistic noise, while
\(k = 3\) and \(k = 4\) hold out to \(n \approx 8\) --- sizes for the
...
becomes statistically unusable for moment estimation at comparable system sizes
--- a statement about this protocol, not about every single-copy strategy.
