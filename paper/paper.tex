...
estimators whose cost grows more slowly in \(n\) than \(M^*\) and falls below it near seven
...
\(0.49\) at \(n = 8\): the pilot budget grows about twofold per qubit against the threshold's
...
points puts the crossing at \(n \approx 7.3\) (Fig.~\ref{fig:pilotratio}). We state this as
measured rather than extrapolated because an earlier fit over \(n \le 6\) alone put it near
\(n = 7\) while understating the required budget by a factor of \(1.85\) at both \(n = 7\) and
\(n = 8\); the crossing survives that correction only because \(M^*\) grows faster than the
pilot does.

The tail of that convergence needs a caveat, because it is where an unbiased estimator can still
mislead. Over the last octave at \(n = 8\) the median absolute deviation falls only from
\(8.4\%\) to \(7.3\%\), flatter than the \(M^{-1/2}\) an unbiased variance estimate should show,
and at the trial counts we can afford the fitted exponent cannot be separated from \(-1/2\) --- so
we do not claim it has been. What can be separated is the mechanism, and it is not an error floor.
\(\widehat{M^*}\) is a \emph{ratio} of two unbiased estimates, so at finite \(M\) it carries a
ratio bias, and at small \(M\) a selection effect as well, from conditioning on
\(\hat\zeta_1 > 0\). A floor would appear as a median offset that stops shrinking; a ratio bias
appears as one that keeps shrinking while dragging the deviation with it. Measured, the median of
\(\widehat{M^*}\) relative to \(M^*\) runs \(0.590,\, 0.876,\, 0.976,\, 1.015,\, 1.026\) across
the five budgets: it converges on unity, crossing it near \(2 \times 10^5\), and the residual
\(7.3\%\) at the largest budget is roughly \(2.6\%\) of surviving median offset on top of a spread
component that is still falling. The practical consequence is a mild one, and we state it as a
limitation rather than discover it later: at the largest sizes a pilot near the \(10\%\) threshold
tends to \emph{under}-estimate \(M^*\), which errs toward believing an experiment is above the
...
becomes relatively cheaper with size and falls below the threshold near seven qubits. What is
